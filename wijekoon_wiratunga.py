# from: https://github.com/rgu-selfback/activity-recognition/blob/master/cnn_svm_keras_activity_recogniser_1d.py
# coding: utf-8
...
class ActivityRecogniser:
    ...
    val_fn = None
    le = None
    feat_ext_methods = {'dct': su.extract_dct_features, 'raw': su.extract_raw_features,
                        '3d_raw': su.extract_3d_raw_features, '3d_dct': su.extract_3d_dct_features,
                        '3d_fft': su.extract_3d_fft_features, '3d_raw_dct': su.extract_3d_raw_dct_features,
                        '3d_raw_fft': su.extract_3d_raw_fft_features}
    ...
    def extract_features(self, train_data, method='dct', samp_rate=100, window_length=10, overlap=0.5, n_comps=None):
        ...
        for data in train_data:
            ...
            for activity in data:
                ...
                #########  Extract Features  #############################################
        X, y = self.feat_ext_methods[method](time_windows, feat_len=(samp_rate * window_length), class_attr='class',
                                             n_comps=n_comps)
        # print X.shape
        return X, y
    ...
    def train_cnn_model(self, X_train, y_train):
        ...
        self.model.add(
            Conv2D(self.params['conv_filter_num'][0], (1, self.params['conv_filter_dims']), activation='relu',
        ...
        for l in range(1, self.params['conv_layers']):
            self.model.add(
                Conv2D(self.params['conv_filter_num'][l], (1, self.params['conv_filter_dims']), activation='relu'))
...
if __name__ == '__main__':
    ...
    for var0 in range(5, 6):
        for var in range(len(instance_ids)):
            ...
            ##### Extract Features #####
            X_train, y_train = ar.extract_features(train_data, window_length=params['win_len'],
                                                   method=params['feat_type'], n_comps=params['n_comps'])
            num_classes = len(set(y_train))
            params['num_classes'] = num_classes
            params['data_depth'] = X_train.shape[1]

            print
            'train data dims: ' + str(X_train.shape)
            ...
            ##### Extact Features #####
            X_test, y_test = ar.extract_features(test_data, window_length=params['win_len'], method=params['feat_type'],
                                                 n_comps=params['n_comps'])
            # X_test, y_test = su.extract_raw_features(test_data)
            print
            'test data dims: ' + str(X_test.shape)
            ...
            csv_data_path = results_path + '_' + str(params['conv_layers']) + 'cnn_svm_knn_' + dataset + '_tw5_' + str(
                params['epochs']) + '_large1.csv'
