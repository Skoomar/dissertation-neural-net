# from: https://github.com/rgu-selfback/OpenEndedHAR/blob/master/w_oe_p_mn.py
import numpy as np
    ...
    for _class in classes:
        if os.path.isdir(path + _class):
            ...
            for f in files:
                ...
                if f.endswith('.csv'):
                    data = []
                    reader = csv.reader(open(os.path.join(path + _class, f), "r"), delimiter=",")
                    next(reader, None)
                    for row in reader:
                        data.append(row)
                    activity_data = {}
...
classes = ["jogging", "sitting", "standing", "walk_fast", "walk_mod", "walk_slow", "upstairs", "downstairs", "lying"]
